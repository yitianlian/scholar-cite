...
def _render_json(papers: list[Paper], formats: list[str]) -> str:
    out = []
    for p in papers:
        cite_dict = {f: getattr(p.citations, f, "") for f in formats}
        out.append(
            {
...
def main(
    ...
    format: str = typer.Option(
        ...
        help="Comma-separated formats or 'all'. "
             "Options: mla, apa, chicago, harvard, vancouver, bibtex, endnote, refman, refworks.",
    ),
    ...
    from scholar_cite.search import search  # lazy import (heavy deps)
